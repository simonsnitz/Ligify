...
def format_results(data_column, ligand_name, data):
    ...
    # Regulator info
    reg_ncbi_anno = data["annotation"]
    reg_protein_seq = data["reg_protein_seq"]
    reg_uniprot_id = data["uniprot_reg_data"]["id"]
    ...
    reg_genbank.header("")
    reg_genbank.header("")
    reg_genbank.form_submit_button(label=f"Show Plasmid for {data['refseq']}", type="primary", on_click=download_df, args=(ligand_name,genbank_con))
    reg_genbank.markdown(f'<p style="font-size: 16px">This plasmid is designed to induce GFP expression in the presence of the target molecule via {refseq}, within E. coli</>', unsafe_allow_html=True)

    # Enzyme info
    enz_annotation = data['protein']['enzyme']['description']
    enz_refseq = data['protein']['enzyme']['ncbi_id']
    equation = data['equation'] 
    rhea_id = data['rhea_id'] 
    references = data['protein']['enzyme']['dois']

    enz_json = {
        "name": "Enzyme attribute",
    ...
    rank_metrics = data["rank"]["metrics"]
    ...
    operon_data = data['protein']['context']
    operon_json = operon_data['operon']
    
    with operon_seq.expander(label=f"Click here to see the genetic context for {refseq}"):
        ...
        enz_index = data['protein']['context']['enzyme_index']
        ...
        def bg_color_col(col):
            ...
            return ['background-color: %s' % color[i] if col.name == 'RefSeq ID' or (col.name == "Description" and i == reg_index) or (col.name == "Description" and i == enz_index) else '' for i, x in col.items()]
        ...
        for seq in operon_data["operon_seq"]:
            ...
            if re.compile(r"spacer").search(seq):
                ...
                c += 1
            operon_seq_html += html
        st.markdown("<h5>Full operon sequence</h5>", unsafe_allow_html=True)
        st.markdown(operon_seq_html, unsafe_allow_html=True)
