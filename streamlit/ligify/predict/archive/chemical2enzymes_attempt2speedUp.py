...
def fetch_genes(output, lineage_filter_name, reviewed_bool):
    ...
    url = "https://rest.uniprot.org/uniprotkb/search?format=json&query=reviewed:"+reviewed+"+AND+"




    rhea_ids = [i["rhea_id"] for i in rxns]
...
if __name__ == "__main__":
    ...
    new_start = time.time()
    output = {}
    output["rxn_data"] = [{"rhea_id": 19965}, {"rhea_id": 72699}, {"rhea_id": 69084}, {"rhea_id": 58984}, {"rhea_id": 34367}, {"rhea_id": 10772}, {"rhea_id": 33367},
    {"rhea_id": 11460}, {"rhea_id": 17825}, {"rhea_id": 19289}, {"rhea_id": 24048}]
    url = "https://rest.uniprot.org/uniprotkb/search?format=json&size=500&query=reviewed:true+AND+"
    rhea_ids = [i["rhea_id"] for i in output["rxn_data"]]
    rhea_suffix = "".join("rhea:"+str(id)+"+OR+" for id in rhea_ids)[:-4]
    ...
    # data = fetch_genes(output, "Family", True)
    url = "https://rest.uniprot.org/uniprotkb/search?format=json&size=500&query=reviewed:true+AND+rhea:"
        # Loop through all RHEA reactions associated with the input chemical.
    length = 0
    for i in range(0,len(output["rxn_data"])):
        response = requests.get(url+str(output["rxn_data"][i]["rhea_id"]))
        data = json.loads(response.text)["results"] 
        length += len(data)
    print(length)
    pprint(data[0])
    ...
    # # length = len([entry for entry in data])

    # # pprint(length)

    print("new query: "+str(new_end - new_start))
    print("old query: "+str(og_stop - new_end))
